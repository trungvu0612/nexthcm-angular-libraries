...
export class ListLeaveEntitlementComponent extends AbstractServerPaginationTableComponent<LeaveEntitlement> {
  readonly leaveConfigAPIUrlPath = LeaveConfigAPIUrlPath.leaveEntitlements;
  ...
  private readonly request$ = this.queryParams$.pipe(
    switchMap(() =>
      this.leaveConfigsService.getConfig<LeaveEntitlement>(this.leaveConfigAPIUrlPath, this.queryParams$.value)
    ),
    map((res) => res.data)
  );
  ...
  private openDialog(label: string, data?: LeaveEntitlement): Observable<LeaveEntitlement> {
    return this.dialogService.open<LeaveEntitlement>(
      new PolymorpheusComponent(UpsertLeaveEntitlementComponent, this.injector),
      {
