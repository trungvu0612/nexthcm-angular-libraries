...
export class OvertimeWorkingComponent implements OnInit {
  ...
  readonly orgIdControl = new FormControl(this.myOrgId);
  ...
  readonly form = this.fb.group({} as WorkingAfterTime);
  ...
  private readonly request$ = this.orgIdControl.valueChanges.pipe(
    filter(isPresent),
    distinctUntilChanged(),
