...
export class HolidayListComponent implements AfterViewInit {
  ...
  readonly columns$: Observable<Columns[]> = this.translocoService
    .selectTranslateObject('HOLIDAY_COLUMNS', {}, this.translocoScope.scope)
    .pipe(
      map((result) => [
    ...
    this.dialogService
      .open<boolean>(new PolymorpheusComponent(UpsertHolidayDialogComponent, this.injector), {
        label: this.translocoService.translate(
          data ? `${this.translocoScope.scope}.editHoliday` : `${this.translocoScope.scope}.addHoliday`
        ),
        size: 'l',
        data,
      })
  ...
  onRemoveHoliday(id: string): void {
    from(
      this.promptService.open({
        icon: 'question',
        html: this.translocoService.translate(`${this.translocoScope.scope}.deleteHoliday`),
      ...
      .subscribe(
        this.promptService.handleResponse(`${this.translocoScope.scope}.deleteHolidaySuccessfully`, () =>
          this.fetch$.next()
        )
      );
  }
